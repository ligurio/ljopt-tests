...
---------------------- END OF PREAMBLE ----------------------------
counter_0 = 0
if only_numbers_cmp(only_numbers_cmp('', '<', ''), '<=',  (setmetatable({ Name0 = '', }, table_mt))() == 1000.000000) then
	end
; 
if '' - '' % only_numbers_cmp('' == not  '', '<', '') then
	elseif false then
	end
; 
...
; 
if only_numbers_cmp(only_numbers_cmp('', '<', ''), '<', only_numbers_cmp('', '<', '')) then
	end
; 
if only_numbers_cmp(only_numbers_cmp('', '<', ''), '<=', only_numbers_cmp(only_numbers_cmp('', '<=', ''), '<', '')) then
	end
; 
